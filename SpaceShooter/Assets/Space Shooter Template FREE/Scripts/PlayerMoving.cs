...
public class PlayerMoving : MonoBehaviour {
    ...
    // 1. Keyboard movement (WASD / Arrow)
    float h = Input.GetAxis("Horizontal");
    float v = Input.GetAxis("Vertical");

    Vector3 keyboardMove = new Vector3(h, v, 0);
    transform.position += keyboardMove * 15f * Time.deltaTime;

    // 2. Mouse movement (hold left click)
